...
\title{SQPOLL \texttt{io\_uring} Against a Device-Saturated Cloud NVMe SSD:\\An Empirical Study}

\author{
  \IEEEauthorblockN{Parth Kumar Sinha\,\orcidlink{0009-0002-3120-9301}}
  \IEEEauthorblockA{Independent Researcher\\
  Oxford, United Kingdom\\
  \texttt{sinhaparth555@gmail.com}\\
  ORCID: \url{https://orcid.org/0009-0002-3120-9301}}
}
...
and tested it against \texttt{libaio} and plain \texttt{io\_uring} (no SQPOLL) on a virtualized
cloud NVMe device across two machine sizes (4 and 16 vCPUs) and a swept queue-depth and
thread-count matrix, alternating 4KB random reads and writes. The argument does not hold once the
device itself saturates: at matched thread count and queue depth, SQPOLL does not beat the
syscall-based baselines on raw IOPS at either machine size once concurrency is high enough to
reach the device's roughly 200K-IOPS ceiling, ruling out core oversubscription as the
explanation for that plateau. Below the plateau, at a single thread, the gap is starker still:
\texttt{libaio} reaches more than double SQPOLL's throughput on the same hardware, a gap that
widens rather than narrows on the wider box. SQPOLL's tail latency also degrades under more
available cores rather than improving. What SQPOLL does deliver is a large reduction in kernel
entries per completed operation, roughly an order of magnitude fewer than plain \texttt{io\_uring}
at high queue depth, at the cost of that added tail latency. Reading this alongside two recent
studies that do report SQPOLL wins in CPU-bound settings, our results are consistent with a
resolution neither we nor they test directly: SQPOLL's advantage is conditional on the CPU or
syscall path being the bottleneck, and it disappears once a saturated device becomes the limit
first. A sweep of \texttt{sq\_thread\_idle} across four orders of magnitude leaves throughput, tail
latency, and kernel entries per operation unchanged, ruling that parameter out as the
explanation: the poller never reaches its idle timeout under this workload. We report the full
methodology, including a page-cache measurement defect that invalidated an earlier round of
results, and discuss what this suite's data does and does not establish about the underlying
cause.
\end{abstract}
...
The engine, the benchmark harness, and the phase-by-phase record of the runs reported here are
at \url{https://github.com/sinhaparth5/ringio}. Both machine sizes ran the same image, \texttt{ubuntu-2204-lts}, kernel 6.8, with
\texttt{liburing} 2.1 as packaged by that image's \texttt{apt} repository.
\texttt{SqpollEngine} sets only two \texttt{io\_uring} setup flags,
\texttt{IORING\_SETUP\_SQPOLL} and, for the shared-poller variant,
\texttt{IORING\_SETUP\_ATTACH\_WQ}; \texttt{IORING\_SETUP\_SINGLE\_ISSUER},
\texttt{IORING\_SETUP\_COOP\_TASKRUN}, and \texttt{IORING\_SETUP\_DEFER\_TASKRUN} are not set
anywhere in this codebase. \texttt{sq\_thread\_idle} is 1000\,ms throughout the main matrix, and swept
separately in Section~V-D. Neither the SQPOLL kernel thread nor any application thread was pinned to a
specific core. NVMe queue count and IOMMU configuration on the underlying GCE Local SSD device
were not recorded and are not user-controllable on this instance type as far as we are aware.
...
\begin{axis}[
    ...
    xtick={libaio,plain io\_uring,SqpollEngine},
...
\addplot[fill=cLibaio, bar shift=0pt] coordinates {(libaio,2.8)};
\addplot[fill=cPlainUring, bar shift=0pt] coordinates {(plain io\_uring,2.8)};
\addplot[fill=cSqpollIndep, bar shift=0pt] coordinates {(SqpollEngine,0.2)};
\end{axis}
\end{tikzpicture}
...
That plateau is also why the single-thread row of Table~\ref{tab:16vcpu-qd32} is the more
informative one, not the 4- and 8-thread rows this section led with. Once the device is
saturated, no submission path can raise IOPS further, so a comparison at the plateau measures
the device, not the interface. Below it, at 1 thread and queue depth 32, \texttt{libaio} reaches
160.7K IOPS while every \texttt{io\_uring}-based backend, SQPOLL included, is stuck at 69--80K:
more than double the throughput from the same single thread, before the device becomes the
limit. That gap, not the shared plateau, is where an interface-level difference is actually
...
\textsuperscript{a}independent-ring variant; \textsuperscript{b}shared-poller variant.}
...
SqpollEngine & 23.5\textsuperscript{a} & 47.6\textsuperscript{b} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{sq\_thread\_idle sensitivity}

The one SQPOLL parameter this study had left fixed is \texttt{sq\_thread\_idle}, the window the
kernel poller spins with no work before sleeping. A poller that sleeps costs a wake-up syscall on
the next submission, so an idle window mistuned for the workload is a plausible source of both
lost throughput and added latency. We swept it across four orders of magnitude, 1\,ms to
10{,}000\,ms, on a second \texttt{n2-standard-16} with Local SSD, holding everything else at the
configuration of Table~\ref{tab:tail-compare} (4 threads, queue depth 128) and adding queue depth
1, where a shallow pipeline gives the poller the most opportunity to time out. Each point is the
median of 7 repetitions; \texttt{libaio}, which the parameter cannot affect, runs in every round
as a drift control.

\begin{table}[t]
\centering
\caption{\texttt{sq\_thread\_idle} sweep at 4 threads, queue depth 128, 16 vCPUs: p99 latency
(ms), shared-poller round trips per operation, and the \texttt{libaio} drift control. Medians of
7 repetitions.}
\label{tab:idle-sweep}
\begin{tabular}{rrrrr}
\toprule
& \multicolumn{2}{c}{SQPOLL p99 (ms)} & shared & \texttt{libaio} \\
\cmidrule(lr){2-3}
idle (ms) & indep. & shared & \texttt{calls\_per\_op} & p99 (ms) \\
\midrule
...
The parameter is inert at every operating point we measured. Throughput does not move: the
shared-poller backend holds 190.8--191.5K IOPS across the whole sweep, a 0.4\% spread, and the
independent-ring backend 197.6--198.7K. The p99 column moves more, 22--40\,ms, but its own
per-point coefficient of variation across repetitions runs 3--27\%, so that spread is inside the
run-to-run noise rather than a trend, and it is not monotonic in the parameter; the
\texttt{libaio} control sits at 12.4--13.5\,ms throughout, confirming the device itself was not
drifting between rounds.

\texttt{calls\_per\_op} is what makes this a mechanism rather than a null measurement. If the
poller were sleeping and being woken, the kernel entries per operation would rise as the idle
window shortened, since a shorter window means more sleeps. It does not move at all: 0.959--0.962
for the shared poller across a $10^4$ range of the parameter, and at queue depth 1, where the
pipeline is shallowest, exactly 1.000 for the shared poller and 4.000 for the independent-ring
variant at every setting tested, with p99 flat at 0.14--0.15\,ms. The poller never times out in
this workload even when given a 1\,ms window, because the submission queue is never empty for
that long. A parameter whose effect is gated on an event that does not occur cannot explain the
tail latency, and we accordingly withdraw it as a candidate.
...
cause. A second candidate was that the SQPOLL tuning constants used throughout this study, fixed
informally against the 4-vCPU box and never retuned for 16, were simply wrong for the wider
machine. The sweep in Section~V-D closes half of that: \texttt{sq\_thread\_idle} does not affect
throughput, tail latency, or kernel entries per operation anywhere in the range tested, because
the poller never reaches its idle timeout under this workload at any of the tested windows. The
completion-side backoff constants (Section~IV-D) remain unswept, and are the one tuning avenue
still open.

We do not treat the contention hypothesis as established. The honest summary of what this suite's data
...
identical-conditions comparison in the buffer-handling dimension. The discussion in Section~VI is
explicitly not settled by this data; the cross-core contention hypothesis has not been tested
with hardware performance counters, and the completion-side backoff constants
(Section~IV-D) were not swept; \texttt{sq\_thread\_idle} was (Section~V-D), and the sweep in
that section was run at 4 threads and queue depths 1 and 128 only, not across the full matrix. Finally, this study compares only SQPOLL, plain
\texttt{io\_uring}, \texttt{libaio}, and POSIX; it does not measure
\texttt{IORING\_SETUP\_IOPOLL} (completion-side polling against a driver configured for it),
NVMe passthrough via \texttt{IORING\_OP\_URING\_CMD}, or SPDK, all of which target the same
per-operation overhead from different angles and are the natural next comparison points.
...
in the Linux man-pages project~\cite{aio-manpages}.

Two more recent studies bear directly on this paper's negative result. Didona et
al.~\cite{didona2022} ran one of the first systematic comparisons of \texttt{libaio}, SPDK, and
\texttt{io\_uring} and found that \texttt{io\_uring}'s polling design has a large effect on
performance, that it can approach SPDK given enough CPU cores, and that scaling across cores and
devices needs a hybrid approach rather than one polling strategy applied uniformly, consistent
with this paper's finding that adding cores did not close SQPOLL's gap on its own. Jasny et
al.~\cite{jasny2025} evaluate \texttt{io\_uring} inside a database buffer manager and report
SQPOLL as one of several incremental wins, roughly 32\% throughput at their operating point,
alongside registered buffers, \texttt{IORING\_SETUP\_IOPOLL}, and NVMe passthrough, in a workload
built to keep the CPU, not the device, as the bottleneck. Both studies point at the same
resolution this paper's data supports without directly testing it: SQPOLL's win condition is a
CPU- or syscall-bound workload, and it disappears once something else, a saturated device in our
case, becomes the limit first. The Linux \texttt{io\_uring\_sqpoll}(7) manual page states this
directly, that SQPOLL "is not universally beneficial and each use case should be benchmarked to
determine if it provides value," and specifically warns against it on CPU-constrained systems and
bursty workloads with long idle gaps~\cite{sqpoll-manpage}. Where this paper differs from all
three is in reporting where an implementation built around the opposite assumption, that SQPOLL
should win outright, actually lands empirically once that assumption is tested against a
device-saturated rather than CPU-saturated setup, including a negative result on raw throughput
and the measurement pitfalls that produced misleading numbers along the way.
...
\texttt{libaio} and plain \texttt{io\_uring} on a real NVMe device. Measured across two machine
sizes and a swept thread-count and queue-depth matrix against a virtualized cloud SSD, it does
not, once that device saturates. \texttt{libaio} matches or beats every SQPOLL configuration we
tested on raw IOPS at every point in the matrix, its advantage is largest, not smallest, below
the device's throughput ceiling, and it grows, not shrinks, on a wider-core machine, ruling out
core oversubscription as the explanation. SQPOLL's actual advantage, an order-of-magnitude
reduction in kernel entries per completed operation at high queue depth, comes with a
tail-latency cost that also grows on the wider machine rather than shrinking. Reading this
against Didona et al.~\cite{didona2022} and Jasny et al.~\cite{jasny2025}, who report SQPOLL
winning in CPU-bound settings this study's device-bound one does not reach, points at a
resolution this paper does not test directly: SQPOLL's value is conditional on where the
bottleneck sits, not a fixed property of the mechanism. The underlying mechanism for the
tail-latency growth remains open, though a sweep of \texttt{sq\_thread\_idle} excludes the
poller's idle window as the cause, and this study does not compare against
\texttt{IORING\_SETUP\_IOPOLL}, NVMe passthrough, or SPDK; this paper reports what was measured
against the four interfaces it did test and separates that plainly from what it did not.

\section*{Declarations}

\textbf{Data availability.} The engine, the benchmark harness, and the phase-by-phase record of
every run reported here, including the configurations behind each table, are at
\url{https://github.com/sinhaparth5/ringio}. Raw Google Benchmark JSON output is not archived;
the harness regenerates it from the commands given in Section~IV.

\textbf{Funding.} This work received no external funding. Cloud instances were self-funded.

\textbf{Conflicts of interest.} The author declares no conflict of interest.

\textbf{Use of AI tools.} Claude Code was used for the benchmark harness implementation, run
orchestration, and manuscript preparation. The author verified all reported measurements and
...
\bibitem{didona2022}
D.~Didona, J.~Pfefferle, N.~Ioannou, B.~Metzler, and A.~Trivedi, ``Understanding modern storage
APIs: a systematic study of libaio, SPDK, and io\_uring,'' in \textit{Proceedings of the 15th ACM
International Conference on Systems and Storage (SYSTOR)}, 2022, pp.~120--127.

\bibitem{jasny2025}
M.~Jasny, M.~El-Hindi, T.~Ziegler, V.~Leis, and C.~Binnig, ``io\_uring for high-performance DBMSs:
when and how to use it,'' \textit{arXiv preprint} arXiv:2512.04859, 2025.

\bibitem{sqpoll-manpage}
\texttt{io\_uring\_sqpoll(7)} manual page, liburing project,
\url{https://manpages.debian.org/testing/liburing-dev/io_uring_sqpoll.7.en.html}.
